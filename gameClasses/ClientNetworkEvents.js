var ClientNetworkEvents = {
	...
	_onCode: function (data) {
		...
		//strdata = String(data.code);

		var hash = b_crc32(data.code);
		//var hash = b_crc32(strdata);
		if (!lessons[hash]) {
			lessons[hash] = data.code;
			$('#lessonList').append('<div class="lessonButton" data-lesson="' + hash + '">' + data.label + '</div>');
		}
	},
...
// CRC32 implementation taken from http://stackoverflow.com/questions/18638900/javascript-crc32
// Certain there is someplace better to put this
...
function b_crc32 (str) {
	...
	for(var i=0, iTop=strString.length; i<iTop; i++) {
		//crc = ( crc >>> 8 ) ^ b_table[( crc ^ str.charCodeAt( i ) ) & 0xFF];
		crc = ( crc >>> 8 ) ^ b_table[( crc ^ strString.charCodeAt( i ) ) & 0xFF];
	}
	return (crc ^ (-1)) >>> 0;
};
